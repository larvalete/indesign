...
function scaleFrameAndContentProportionally(frame, targetWidth) {
    ...
    // Determine the scaling factor as a percentage
    var scaleFactor = (targetWidth / currentWidth) * 100;

    // Apply the scaling to the frame, which includes its content
    // This scales both the frame and its content proportionally
    frame.transform(
        CoordinateSpaces.PASTEBOARD_COORDINATES, // Transform in the context of the pasteboard
        AnchorPoint.CENTER_ANCHOR,               // Use the center as the anchor point for scaling
        new Scale(scaleFactor, scaleFactor, true) // Scale both horizontally and vertically
    );
}
